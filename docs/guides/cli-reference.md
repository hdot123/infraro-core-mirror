...
`infra-daily-audit` 的实现为门面模块：`infra_core.packs.memory.daily_audit` 显式
re-export 全部既有导出名（含下划线别名），实体代码在 5 个平铺私有模块
（`_daily_base` 常量/路径/工具、`_daily_checks` 检查 1-5、`_daily_infra` 基础设施、
`_daily_report` 报告与通知、`_daily_cli` CLI 编排），依赖方向单向
cli → report → infra/checks → base。
