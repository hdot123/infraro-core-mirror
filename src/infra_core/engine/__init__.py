...
寄居件警告（四层归类声明，权威表见 docs/architecture.md §1.1）：
``droid_review/``、``anchor_gate.py``、``extract_anchor.py`` 物理寄居本目录，
但**逻辑归 Delivery 层**（交付件），不是 Evolution Core 的一部分。本包核心
``evolution_*`` 模块**禁止 import 它们**——单向依赖原则为
``Delivery → Evolution Core``，严禁 Core 反向依赖 Delivery。寄居原因：搬走
它们会撕裂 suppress 指纹、gate0 豁免、消费锚定与字节锁（机械登记见
docs/governance/source-of-truth.md §5）。
"""
