...
# Extract the bare branch name from a protected-list entry. Entries carry a
# " (N unique commits)" suffix minted by branch_cleanup.sh; plain names pass
# through unchanged.
branch_name_of() {
  sed -E 's/ \([0-9]+ unique commits\)$//' <<<"$1"
}

# Staleness guard (Issue #129 race): the cleanup step computes the protected
# list, then THIS step writes the tracking issue — a branch deleted inside
# that window (immediate mode: auto-merge deletes the merged branch seconds
# after PR merge, racing the tracker step) would be recorded as a phantom
# protected entry. Before a tracker CREATE, and before an UPDATE writes newly
# protected entries, revalidate candidates against the remote and drop
# confirmed-absent branches. Unchanged runs (reused-silent) skip revalidation.
remote_branch_exists() {
  # git ls-remote --exit-code: 0 = ref found; 2 = no matching refs (confirmed
  # absent); anything else = the check itself failed (network, no repo) —
  # callers keep the branch then (fail-open: never lose records to our own
  # outage). set -e note: guarded by || rc=$?.
  local branch="$1" rc=0
  git ls-remote --exit-code --heads origin "${branch}" >/dev/null 2>&1 || rc=$?
  if [[ "$rc" -eq 0 ]]; then
    return 0 # exists on remote
  fi
  if [[ "$rc" -eq 2 ]]; then
    return 1 # confirmed absent
  fi
  return 2 # check failed — indeterminate
}

# Print, one per line, the entries from stdin that survive remote
# revalidation (deduplicated). Logs each confirmed-absent drop and each
# indeterminate keep to STDERR — stdout carries only surviving entries so the
# function composes cleanly inside command substitutions. Callers feed
# sort -u'd input; output preserves order.
revalidate_entries() {
  local entry name rc
  while IFS= read -r entry; do
    [[ -z "$entry" ]] && continue
    name=$(branch_name_of "$entry")
    rc=0
    remote_branch_exists "$name" || rc=$?
    if [[ "$rc" -eq 1 ]]; then
      echo "staleness_guard: dropped ${name} (no longer on remote)" >&2
      continue
    fi
    if [[ "$rc" -eq 2 ]]; then
      echo "staleness_guard: remote check failed for ${name} — keeping (fail-open)" >&2
    fi
    echo "$entry"
  done
  return 0
...
# CREATE-path guard: revalidate the whole protected list in place (deduped,
# each branch checked exactly once) and recompute PROTECTED_COUNT so routing
# sees the filtered state; when the list empties and DELETED_COUNT=0 the
# caller takes the existing zero-actionable path.
drop_stale_protected() {
  if [[ -z "$PROTECTED_ITEMS" ]]; then
    return 0
  fi
  PROTECTED_ITEMS=$(revalidate_entries <<< "$(echo "$PROTECTED_ITEMS" | sort -u)")
...
# ---------------------------------------------------------------------------
# Build the report section for the current run (function, not inline: the
# create path builds it AFTER the staleness guard, the update path after the
# diff guard — dropped branches must never enter a body/comment).
# ---------------------------------------------------------------------------
build_report() {
...
if [[ -z "$TRACKER_URL" ]]; then
  # No open tracking issue: create the single reusable one. Revalidate the
  # protected list first (Issue #129): branches deleted between the cleanup
  # step and this write must not enter a fresh tracker.
  drop_stale_protected
  if [[ "$DELETED_COUNT" -eq 0 && "$PROTECTED_COUNT" -eq 0 ]]; then
    echo "All candidates stale: nothing actionable for a new tracker."
    echo "issue_action=none"
    exit 0
  fi
...
# UPDATE-path staleness guard (Issue #129): revalidate only the entries about
# to be newly written (ADDED_PROTECTED); confirmed-absent branches are dropped
# from both the comment and the body below. DELETED-count entries are not
# revalidated (they report deletions, not intent to track). REMOVED_PROTECTED
# (tracker entries absent from this run's list) is a pure diff artifact and
# needs no remote check.
if [[ -n "$ADDED_PROTECTED" ]]; then
  ADDED_PROTECTED=$(revalidate_entries <<< "$(echo "$ADDED_PROTECTED" | sort -u)")
fi

# A drop may collapse the update into "no change at all": ADDED emptied, no
# REMOVED, no deletions -> fall through to silent reuse instead of writing an
# empty comment. When REMOVED/deletions remain the update still reports them.
if [[ -z "$ADDED_PROTECTED" && -z "$REMOVED_PROTECTED" && "$DELETED_COUNT" -eq 0 ]]; then
  echo "Added entries stale (deleted before tracker write): no comment needed."
  close_duplicate_trackers
  echo "issue_action=reused-silent"
  exit 0
fi

# Drop stale added entries from the sets the body/comment are built from.
# When entries survived or nothing was added, PROTECTED_ITEMS is already the
# correct body set (set arithmetic: PROTECTED = TRACKED - REMOVED + ADDED);
# only a partial ADDED drop requires recomposing from TRACKED ∪ survivors.
if [[ -n "$ADDED_PROTECTED" ]]; then
  CURRENT_PROTECTED=$(printf '%s\n%s' "$TRACKED_PROTECTED" "$ADDED_PROTECTED" | sort -u)
fi
