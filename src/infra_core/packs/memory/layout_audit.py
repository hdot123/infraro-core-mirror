...
Migrated from memory-core ``memory_core/tools/audit_project_layout.py`` into
the infra-core memory rule pack. Behaviour is kept equivalent; the only
adaptations are:

- ownership primitives come from ``.ownership_reader`` instead of
  ``memory_core.ownership`` (no memory_core dependency)
- ``classify_owned_path`` is injectable (``Callable[[str], Owned | None]``);
  when not injected it defaults to ``ownership_reader.classify_owned_path``
- ``--repo-root`` is accepted as an alias of ``--target``
- ``validate_ownership_schema`` is provided locally against the memory-core
  default domain set (resources are not parsed by ownership_reader, so
  resource-level validation does not apply under the default classifier)

Usage:
    infra-layout-audit --target /path/to/project
    infra-layout-audit --repo-root /path/to/project --json
    infra-layout-audit --target /path/to/project --json --severity P0
...
from . import ownership_reader as _default_ownership

# Injected classifier contract: takes a project-relative path (forward
# slashes) and returns an ``Owned`` instance when the path is owned/protected,
# ``None`` when it is not. memory-core consumers may inject their own
# ``memory_core.ownership.classify_owned_path``-based callable for semantic
# parity; the anti-drift contract test asserts identical outputs.
ClassifyOwnedPath = Callable[[str], Any]
    ...
    if current_structures and workspace_structures:
        ...
        result.findings.append(
            Finding(
                ...
                path=root.name,
                message=(
                    f"Root memory structures conflict with workspace legacy: {', '.join(locations)}"
                ),
                suggested_bucket="needs_human_decision",
            )
        )
...
# Default ownership domains/resources mirroring memory-core's
# DEFAULT_OWNERSHIP_DOMAINS / DEFAULT_OWNERSHIP_RESOURCES. Used as the
# reference baseline for schema weakening checks; ownership_reader does not
# parse [[resources]], so resources are read from the TOML directly here.
_DEFAULT_DOMAIN_LEVELS: dict[str, str] = {
    "memory_docs": "critical",
    "memory_kb": "critical",
    "memory_system": "critical",
    "project_map": "critical",
    "audit": "critical",
    "review": "critical",
    "memory_log": "standard",
}

_DEFAULT_RESOURCE_LEVELS: dict[str, str] = {
    "agents_md": "critical",
    "audit_summary": "critical",
    "readme_md": "critical",
    "changelog_md": "critical",
    "contributing_md": "critical",
    "memory_lock": "critical",
    "adapter_toml": "critical",
    "ownership_toml": "critical",
    "migrations_log": "standard",
    "manifest_json": "critical",
    "error_log": "standard",
}

# least -> most strict
_LEVEL_ORDER = {"recommended": 0, "standard": 1, "critical": 2}


def _load_ownership_toml_raw(root: Path) -> dict[str, Any] | None:
    """Parse memory/system/ownership.toml into a raw dict.

    Returns:
        None if the file is missing or unparseable (memory-core's loader
        falls back to defaults in that case - no weakening to report);
        the parsed dict otherwise (``{}`` for a valid-but-empty file, which
        memory-core validates against defaults and flags as fully deleted).
    """
...
def _validate_ownership_schema(data: dict[str, Any]) -> list[str]:
    """Validate an ownership config for schema weakening.

    Local equivalent of ``memory_core.ownership.validate_ownership_schema``
    working on the raw parsed TOML (domains + resources). Detects:

    - Deleted default domains / resources
    - Downgraded protection levels

    Note: memory-core additionally reports non-recursive critical domains,
    but those errors match neither the "downgraded" nor "deleted" branch in
    ``_check_ownership_findings`` and are silently dropped there, so they are
    intentionally not produced here.
    """
    errors: list[str] = []

    domains = [d for d in data.get("domains", []) if isinstance(d, dict)]
    resources = [r for r in data.get("resources", []) if isinstance(r, dict)]

    current_domain_names = {str(d.get("name", "")) for d in domains}
    missing_domains = set(_DEFAULT_DOMAIN_LEVELS) - current_domain_names
    if missing_domains:
        errors.append(f"Deleted default domains: {', '.join(sorted(missing_domains))}")

    current_resource_names = {str(r.get("name", "")) for r in resources}
    missing_resources = set(_DEFAULT_RESOURCE_LEVELS) - current_resource_names
    if missing_resources:
        errors.append(f"Deleted default resources: {', '.join(sorted(missing_resources))}")

    for current in domains:
        name = str(current.get("name", ""))
        default_level = _DEFAULT_DOMAIN_LEVELS.get(name)
        if default_level is None:
            continue
        level = str(current.get("level", "standard")).lower()
        if _LEVEL_ORDER.get(level, 1) < _LEVEL_ORDER[default_level]:
            errors.append(
                f"Domain '{name}' downgraded from {default_level.upper()} to {level.upper()}"
            )

    for current in resources:
        name = str(current.get("name", ""))
        default_level = _DEFAULT_RESOURCE_LEVELS.get(name)
        if default_level is None:
            continue
        level = str(current.get("level", "standard")).lower()
        if _LEVEL_ORDER.get(level, 1) < _LEVEL_ORDER[default_level]:
            errors.append(
                f"Resource '{name}' downgraded from {default_level.upper()} to {level.upper()}"
            )
...
def _resolve_classifier(
    target: Path,
    classify_owned_path: ClassifyOwnedPath | None,
) -> ClassifyOwnedPath:
    """Resolve the effective classifier for a target project.

    Returns the injected classifier when provided; otherwise builds a
    default classifier from ``ownership_reader.classify_owned_path`` bound
    to the target's ownership configuration. When the target has no
    ownership.toml (or it is unparseable), a default-domain configuration is
    built so the classifier still recognizes standard owned paths - this
    mirrors memory-core's ``load_memory_ownership`` fallback-to-defaults.
    """
    if classify_owned_path is not None:
        return classify_owned_path

    config = _default_ownership.load_ownership_toml(target)
    if not config.domains:
        domain_paths = {
            "memory_docs": "memory/docs",
            "memory_kb": "memory/kb",
            "memory_system": "memory/system",
            "project_map": "memory/project-map",
            "audit": "audit",
            "review": "review",
            "memory_log": "memory/log",
        }
        domains = [
            _default_ownership.OwnedDomain(
                name=name,
                path=domain_paths[name],
                level=level,
                recursive=True,
                description=f"Default {name} ownership domain (memory-core parity)",
            )
            for name, level in _DEFAULT_DOMAIN_LEVELS.items()
        ]
        config = _default_ownership.OwnershipConfig(domains=domains)

    def _default_classifier(rel_path: str) -> _default_ownership.Owned | None:
        result = _default_ownership.classify_owned_path(rel_path, ownership=config)
        return result if isinstance(result, _default_ownership.Owned) else None
...
def audit_project_layout(
    ...
    classify_owned_path: ClassifyOwnedPath | None = None,
) -> AuditResult:
    ...
    Args:
        ...
        classify_owned_path: Optional injected ``Callable[[str], Owned | None]``
            classifier. When None (default), ``ownership_reader.classify_owned_path``
            is used. Accepted for API symmetry with ``plan_residue_migration``;
            the audit path itself validates ownership.toml rather than
            classifying individual paths.
    ...
    if severity_filter:
        ...
        result.findings = [
            f for f in result.findings if severity_order.get(f.severity, 2) <= min_level
        ]
...
def _check_ownership_findings(root: Path, result: AuditResult) -> None:
    ...
    if not ownership_path.exists():
        ...
        try:
            data = _load_ownership_toml_raw(root)
            if data is not None:
                schema_errors = _validate_ownership_schema(data)
...
def _calculate_risk_level(
    ...
    conflict_p0_count = len(
        [f for f in findings if f.severity == "P0" and f.kind == "multi_generation_conflict"]
    )
...
def _generate_must_commit_together(findings: list[Finding]) -> list[list[str]]:
    ...
    for finding in findings:
        if (
            finding.kind in ("agents_md_marked", "agents_md_unmarked")
            or finding.kind == "index_md_business"
        ):
...
def _check_root_vs_workspace_conflict(
    ...
    """
    has_root_memory = any(
        f.kind in ("dot_memory", "current_memory", "project_map") for f in findings
    )
    has_workspace_memory = any(
        f.kind in ("workspace_memory", "workspace_project_map") for f in findings
    )
                ...
                buckets["root_pollution"].append(
                    {
                        ...
                        "kind": "root_docs_symlink",
                        "message": (
                            f"Root-level document symlink is forbidden; use memory/docs/: {name}"
                        ),
                    }
                )
                actions.append(
                ...
                buckets["root_pollution"].append(
                    {
                        ...
                        "kind": "root_docs_dir",
                        "message": (
                            f"Root-level document directory is forbidden; use memory/docs/: {name}"
                        ),
                    }
                )
                actions.append(
...
def _populate_forbidden_overwrites(
    target: Path,
    classify_owned_path: ClassifyOwnedPath | None = None,
) -> list[str]:
    """Populate forbidden_overwrites list using classify_owned_path.

    Uses the injected classifier when provided; otherwise defaults to
    ``ownership_reader.classify_owned_path`` bound to the target's
    ...
    forbidden: list[str] = []
    try:
        classifier = _resolve_classifier(target, classify_owned_path)
        # Check all files in the project
        for item in target.rglob("*"):
            if item.is_file():
                try:
                    rel_path = item.relative_to(target).as_posix()
                    classification = classifier(rel_path)
...
def plan_residue_migration(
    ...
    Args:
        audit_result: Audit findings from ``audit_project_layout``.
        target: Path to the project root being analyzed.
        classify_owned_path: Optional injected ``Callable[[str], Owned | None]``
            classifier used to compute ``forbidden_overwrites``. When None
            (default), ``ownership_reader.classify_owned_path`` is used.
    ...
    forbidden_overwrites = _populate_forbidden_overwrites(target, classify_owned_path)
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Audit project memory layout and detect legacy/residue patterns."
    )
    ...
    target_group.add_argument(
        ...
        type=Path,
        default=None,
        help="Alias of --target: path to the project root to audit.",
    )
    parser.add_argument(
        "--json",
        action="store_true",
    ...
    target = args.target if args.target is not None else args.repo_root

    result = audit_project_layout(target, severity_filter=args.severity)
...
def plan_main(argv: list[str] | None = None) -> int:
    """Entry point for the residue migration plan CLI."""
    parser = argparse.ArgumentParser(
        description="Generate migration plan for memory residue cleanup."
    )
    target_group = parser.add_mutually_exclusive_group(required=True)
    target_group.add_argument(
        "--target",
    ...
    target_group.add_argument(
        ...
        type=Path,
        default=None,
        help="Alias of --target: path to the project root to analyze.",
    )
    parser.add_argument(
        "--json",
        action="store_true",
    ...
    plan = plan_residue_migration(audit_result, target)
