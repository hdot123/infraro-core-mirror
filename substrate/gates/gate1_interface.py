...
``hdot123/infraro`` via ``gh api`` (charter round-3 item A4).
...
def fetch_decl_templates() -> dict[str, str]:
    ...
    if local is not None:
        ...
        if templates.is_dir():
            return {p.name: p.read_text(encoding="utf-8") for p in sorted(templates.glob("*.yml"))}
    listing = gh_api(f"repos/{DECL_REPO_SLUG}/contents/templates")
    if isinstance(listing, list):
        templates: dict[str, str] = {}
        for item in listing:
            url = item.get("download_url")
            if item.get("name", "").endswith(".yml") and url:
                with urllib.request.urlopen(url, timeout=30) as resp:  # noqa: S310
                    templates[item["name"]] = resp.read().decode("utf-8")
        return templates
    return {}
...
def main() -> int:
    ...
    checked = 0
    if not templates:
        print("WARN: declaration templates unavailable (no sibling checkout, gh api failed)")
        print("LOCAL-ONE: template face not verifiable in this environment; recorded in registry")
        return 0
