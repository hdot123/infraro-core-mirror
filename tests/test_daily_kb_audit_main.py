...
def _patch_infra_clean():
    ...
    with patch(
        "infra_core.packs.memory.daily_audit.check_infrastructure",
...
def _patch_no_projects():
    ...
    with patch(
        "infra_core.packs.memory.daily_audit.load_registered_projects",
        return_value=[],
    ):
        yield
...
def _patch_fingerprints():
    ...
    with patch(
        "infra_core.packs.memory.daily_audit.build_global_kb_fingerprints",
        return_value={},
    ):
        yield
...
def _patch_audit_project_clean():
    ...
    with patch(
        "infra_core.packs.memory.daily_audit.audit_project",
...
def _patch_write_report(tmp_path: Path):
    ...
    with patch("infra_core.packs.memory.daily_audit.write_report", side_effect=_write):
        yield out
...
def _patch_build_report():
    ...
    with patch("infra_core.packs.memory.daily_audit.build_report", side_effect=_build):
        yield
...
def _patch_notify():
    ...
    with patch(
        "infra_core.packs.memory.daily_audit.notify_via_lark",
        return_value=True,
    ) as mock_notify:
        yield mock_notify
...
class TestMainProjectAuditException:
    ...
    def test_single_project_exception_captured(
        ...
        with patch(
            ...
            def _audit_side_effect(name, root, fps, expected_version=None):
                nonlocal call_count
                call_count += 1
                if name == "proj-a":
...
class TestMainNoWrite:
    ...
    def test_no_write_skips_write_report(
        ...
        with patch(
            "infra_core.packs.memory.daily_audit.write_report",
        ) as mock_write:
            main(["--no-write"])
