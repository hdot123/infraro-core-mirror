...
class TestWorkflowCallSurface:
    ...
    def test_secrets_declared(self, workflow_call):
        ...
        2026-09-21 go-github 切换：注入面从 factory_api_key/nvidia_kong_proxy_key
        改为 factory_api_key/opencode_go_key/go_github_run_token（DeepSeek
        V4.1 Flash 链路，GHA secrets 上下文大小写不敏感，消费侧 env 变量名
        保持脚本契约大小写，仅声明/转发层 snake 化）。
        """
        ...
        assert "opencode_go_key" in secrets, "必须声明 opencode_go_key 注入面（Go 订阅 key）"
        assert "go_github_run_token" in secrets, "必须声明 go_github_run_token 注入面（网关认证）"
        for name in secrets:
            assert "-" not in name and name == name.lower(), (
