...
def test_scanner_recent_failure_still_alive():
    """1-2 recent failures → still 'alive' (it ran, just errored).

    The age rule detects if the scanner STOPPED, not if it errored once or
    twice. Only a sustained all-failure streak (see
    test_scanner_three_consecutive_failures_stale) flips liveness to stale.
    """
    runs = [_recent_run(0.5, "failure"), _recent_run(1.5, "failure")]
    with patch("evolution_heartbeat.subprocess.run") as mock_run:
        mock_run.return_value = _gh_result(json.dumps(runs))
        result = evolution_heartbeat.check_scanner_liveness()
...
# ---------------------------------------------------------------------------
# VAL-HB-001/002: conclusion dimension — a fresh history that only failed
# ---------------------------------------------------------------------------


def test_consecutive_failure_staleness_locked_to_three():
    """N=3, aligned with GRACE_PERIOD_TICKS (evolution_utils) — not a tuning knob."""
    assert evolution_heartbeat.CONSECUTIVE_FAILURE_STALENESS == 3


def test_scanner_three_consecutive_failures_stale():
    """Newest 3 runs all concluded 'failure' → stale even though createdAt is fresh.

    Age alone called this alive (blind spot: a cron that only errors never
    produces results). Runs older than the 3-run window must not matter.
    """
    runs = [
        _recent_run(0.5, "failure"),
        _recent_run(2.0, "failure"),
        _recent_run(3.5, "failure"),
        _recent_run(5.0, "success"),
    ]
    with patch("evolution_heartbeat.subprocess.run") as mock_run:
        mock_run.return_value = _gh_result(json.dumps(runs))
        result = evolution_heartbeat.check_scanner_liveness()
...
def test_scanner_two_failures_one_success_alive():
    """2 failures + 1 success in the newest 3 → age rule keeps it alive (no false positive)."""
    runs = [
        _recent_run(0.5, "failure"),
        _recent_run(1.5, "failure"),
        _recent_run(2.5, "success"),
    ]
    with patch("evolution_heartbeat.subprocess.run") as mock_run:
        mock_run.return_value = _gh_result(json.dumps(runs))
        result = evolution_heartbeat.check_scanner_liveness()
...
def test_scanner_fewer_than_three_runs_keeps_age_rule():
    """Run history shorter than the streak window → age rule only (not enough evidence)."""
    runs = [_recent_run(0.5, "failure"), _recent_run(1.5, "failure")]
    with patch("evolution_heartbeat.subprocess.run") as mock_run:
        mock_run.return_value = _gh_result(json.dumps(runs))
        result = evolution_heartbeat.check_scanner_liveness()
...
def test_scanner_three_failures_with_pending_newest_alive():
    """Newest run still in progress (conclusion None) breaks the streak → age rule.

    Non-failure conclusions (success/neutral/pending/None) all keep the age rule.
    """
    pending = _recent_run(0.3)
    pending.update(status="in_progress", conclusion=None)
    runs = [pending, _recent_run(1.5, "failure"), _recent_run(2.5, "failure")]
    with patch("evolution_heartbeat.subprocess.run") as mock_run:
        mock_run.return_value = _gh_result(json.dumps(runs))
        result = evolution_heartbeat.check_scanner_liveness()

    assert result["alive"] is True
    assert result["last_status"] == "in_progress"


def test_scanner_three_failures_with_neutral_newest_alive():
    """A neutral conclusion in the newest 3 also keeps the age rule."""
    runs = [
        _recent_run(0.5, "neutral"),
        _recent_run(1.5, "failure"),
        _recent_run(2.5, "failure"),
    ]
    with patch("evolution_heartbeat.subprocess.run") as mock_run:
        mock_run.return_value = _gh_result(json.dumps(runs))
        result = evolution_heartbeat.check_scanner_liveness()
...
def test_heartbeat_workflow_three_consecutive_failures_stale():
    """Same shared probe → the heartbeat reverse-watch sees the streak too (VAL-HB-001)."""
    runs = [
        _recent_run(0.5, "failure"),
        _recent_run(2.0, "failure"),
        _recent_run(3.5, "failure"),
...
# ---------------------------------------------------------------------------
# F5: conclusion_streak penetration through self-heal suppression
# ---------------------------------------------------------------------------


def test_conclusion_streak_penetrates_dispatch_suppression():
    """F5: 3 consecutive failures + dispatch accepted + fresh age → rc==1 + alert created.

    Root cause of INFRA-597 age-type silence: 3x failure streaks were getting
    suppressed by the self-heal mechanism. The failure streak should penetrate
    the suppression chain so alerts are created when there are 3 consecutive
    failures regardless of dispatch acceptance.
    """
    # Create 3 runs all with "failure" conclusion within the CONSECUTIVE_FAILURE_STALENESS window
    runs = [
        _recent_run(0.5, "failure"),  # newest
        _recent_run(1.5, "failure"),
        _recent_run(2.5, "failure"),  # oldest of the 3
    ]
    ...
    # Should return 1 (alert condition) and create alert despite dispatch being accepted
    assert rc == 1
    mock_create.assert_called_once()
    kwargs = mock_create.call_args.kwargs
    assert kwargs["scanner_stale"] is True
...
def test_conclusion_streak_keeps_existing_scanner_stale_alert_open():
    """F5: streak scenario keeps existing scanner_stale alert open (no close call).

    With failure streak + dispatch accepted, effective_liveness.alive=False keeps
    scanner_stale in current_anomalies, so resolve_cleared_alerts does NOT close
    the existing alert. The test verifies:
    - rc == 1 (alert condition: stale_alertable=True due to streak)
    - No gh issue close call is made (existing alert remains open)
    """
    runs = [
        _recent_run(0.5, "failure"),
        _recent_run(1.5, "failure"),
    ...
    open_alerts = [{"number": 1001, "body": _ALERT_BODY_SCANNER_STALE}]
    ...
    with (
        ...
        patch("evolution_heartbeat.list_open_alert_issues", return_value=open_alerts),
        patch("evolution_heartbeat.trigger_scanner_dispatch", return_value=(True, None)),
        patch("evolution_heartbeat.alert_issue_exists", return_value=True),  # Existing alert
        patch("evolution_heartbeat.write_monitor_heartbeat"),
    ):
        # Mock liveness to return a failure streak
        mock_liveness.return_value = {
            "alive": False,
            "hours_since_last_run": 0.5,
            "liveness_data_ok": True,
            "last_status": "failure",
            "conclusion_streak": True,  # F5: failure streak
            "message": "Scanner stale: last 3 runs concluded failure",
        }

        # Set up the subprocess calls properly for the various commands
        def side_effect(args, **kwargs):
            if "issue" in args and "view" in args:
                # Return issue with no self-heal comment
                return _gh_result(stdout=json.dumps({"comments": []}))
    ...
    # After fix: rc == 1 (streak + dispatch accepted → alertable → return 1)
    assert rc == 1, f"Expected rc==1 (alert condition), got {rc}"

    # Verify no gh issue close call is made (existing alert remains open)
    close_calls = [
        call for call in mock_run.call_args_list if "issue" in str(call) and "close" in str(call)
    ]
    assert len(close_calls) == 0, f"Expected no 'issue close' calls, but got {len(close_calls)}"


def test_timed_out_streak_marks_stale():
    """D4: 3 consecutive timed_out conclusions → stale."""
    runs = [
        _recent_run(0.5, "timed_out"),  # newest
        _recent_run(1.5, "timed_out"),
        _recent_run(2.5, "timed_out"),  # oldest of the 3
    ]
    ...
    assert result["alive"] is False
    assert "stale" in result["message"].lower()
    assert "timed_out" in result["message"] or "failure" in result["message"]


def test_cancelled_streak_marks_stale():
    """D4: 3 consecutive cancelled conclusions → stale."""
    runs = [
        _recent_run(0.5, "cancelled"),  # newest
        _recent_run(1.5, "cancelled"),
        _recent_run(2.5, "cancelled"),  # oldest of the 3
    ]
...
def test_mixed_failure_conclusions_streak_marks_stale():
    """D4: mixed failure/timed_out/cancelled in 3-run window → stale."""
    runs = [
        _recent_run(0.5, "failure"),  # newest
        _recent_run(1.5, "timed_out"),
        _recent_run(2.5, "cancelled"),  # oldest of the 3
...
def test_success_in_window_breaks_streak():
    """F5 regression: success in the 3-run window breaks the failure streak → alive."""
    runs = [
        _recent_run(0.5, "success"),  # newest
        _recent_run(1.5, "failure"),
        _recent_run(2.5, "failure"),  # Would be streak if not for the success at front
    ]
    ...
    # With success in the window, it should not be considered a failure streak,
    # so should fall back to age-based assessment
    assert result["alive"] is True  # Since most recent run is success and fresh
