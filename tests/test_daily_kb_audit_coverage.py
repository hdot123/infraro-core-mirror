...
class TestLoadRegisteredProjects:
    def test_returns_empty_when_no_index(self, tmp_path: Path):
        result = load_registered_projects(tmp_path / "nope.json")
        assert result == []
    ...
    def test_returns_empty_on_invalid_json(self, tmp_path: Path):
        ...
        assert load_registered_projects(idx) == []
    ...
    def test_parses_valid_index(self, tmp_path: Path):
        ...
        result = load_registered_projects(idx)
        assert len(result) == 1
...
class TestBuildGlobalKbFingerprints:
    def test_empty_when_no_root(self, tmp_path: Path):
        assert build_global_kb_fingerprints(tmp_path / "no-global") == {}
    ...
    def test_builds_fingerprints(self, tmp_path: Path):
        ...
        fps = build_global_kb_fingerprints(tmp_path)
        assert len(fps) >= 1
...
class TestCheckVersionConsistency:
    ...
    def test_mismatched_version(self, tmp_path: Path):
        ...
        # Pass expected_version to trigger mismatch detection (adapted for infra-core refactor)
        violations = check_version_consistency(tmp_path, expected_version="1.0.0")
        assert len(violations) == 3
        assert all("不一致" in v["detail"] for v in violations)
...
class TestTcpConnectOk:
    ...
    @patch("infra_core.packs.memory._daily_infra.socket.create_connection")
...
class TestCheckSshReachable:
    @patch("infra_core.packs.memory._daily_infra._run_ssh")
...
class TestCheckServer:
    @patch("infra_core.packs.memory._daily_infra._tcp_connect_ok")
    @patch("infra_core.packs.memory._daily_infra.check_ssh_reachable")
...
class TestCheckDatabase:
    ...
    def test_db_reachable(self, mock_tcp):
        ...
        global_v: list[dict] = []
        db = {"name": "mydb", "host": "[REDACTED-IP]", "port": 3306}
    ...
    def test_unsupported_check_kind(self):
        global_v: list[dict] = []
        db = {"name": "mydb", "host": "[REDACTED-IP]", "port": 3306, "check": "mysql_ping"}
        result = check_database(db, global_v)
        assert result["connect_ok"] is False
...
class TestCheckInfrastructure:
    def test_returns_empty_when_no_inventory(self, tmp_path: Path):
        with patch(
            "infra_core.packs.memory._daily_infra._load_infra_inventory",
            return_value=None,
        ):
    ...
    @patch("infra_core.packs.memory._daily_infra.check_database")
    @patch("infra_core.packs.memory._daily_infra.check_server")
    @patch("infra_core.packs.memory._daily_infra._load_infra_inventory")
    def test_processes_servers_and_dbs(self, mock_load, mock_server, mock_db):
        mock_load.return_value = {
            "servers": [{"name": "srv1", "host": "[REDACTED-IP]"}],
            "databases": [{"name": "db1", "host": "[REDACTED-IP]", "port": 3306}],
        }
        mock_server.return_value = {"host": "[REDACTED-IP]", "violations": []}
        mock_db.return_value = {"host": "[REDACTED-IP]", "connect_ok": True, "violations": []}
        result = check_infrastructure()
...
class TestWriteReport:
    def test_writes_file(self, tmp_path: Path):
        ...
        out = write_report(report, audit_dir=tmp_path)
        assert out.exists()
...
class TestNotifyViaLark:
    ...
    @patch("infra_core.packs.memory._daily_report.subprocess.run")
...
class TestMain:
    @patch("infra_core.packs.memory._daily_cli.check_infrastructure")
    @patch("infra_core.packs.memory._daily_cli.load_registered_projects")
    @patch("infra_core.packs.memory._daily_cli.write_report")
    ...
    @patch("infra_core.packs.memory._daily_cli.build_global_kb_fingerprints")
    @patch("infra_core.packs.memory._daily_cli.audit_project")
    @patch("infra_core.packs.memory._daily_cli.check_infrastructure")
    @patch("infra_core.packs.memory._daily_cli.load_registered_projects")
    @patch("infra_core.packs.memory._daily_cli.write_report")
    ...
    @patch("infra_core.packs.memory._daily_cli.notify_via_lark")
    @patch("infra_core.packs.memory._daily_cli.build_global_kb_fingerprints")
    @patch("infra_core.packs.memory._daily_cli.audit_project")
    @patch("infra_core.packs.memory._daily_cli.check_infrastructure")
    @patch("infra_core.packs.memory._daily_cli.load_registered_projects")
