...
class GhMockHarness:
    ...
    def __init__(
        ...
        ls_remote_rc_overrides: dict[str, int] | None = None,
    ) -> None:
        """Args:
        ...
        curl_responses: canned responses for the mock curl (consumed in order,
            ...
            real Linear API.
        remote_branches: branch names reported as existing on the remote by
            the mock git (staleness-guard tests). Default: every queried
            branch "exists" (rc 0).
        ls_remote_rc_overrides: per-branch rc for the mock git ls-remote
            (staleness-guard tests); wins over remote_branches. rc 0 = exists,
            rc 2 = confirmed absent, other = check failed.
        """
        ...
        self.ls_remote_rc_overrides = ls_remote_rc_overrides or {}
        ...
        # Mock git: records ls-remote invocations (staleness guard, Issue #129)
        # and answers from remote_branches / ls_remote_rc_overrides. Only
        # `ls-remote` is intercepted; any other git argv falls through to the
        # real git (none is expected — the script only uses ls-remote here).
        self.git_calls_file = tmp_path / "git_calls.jsonl"
        self.git_calls_file.write_text("")
        mock_git = mock_dir / "git"
        mock_git.write_text(
            "#!/usr/bin/env python3\n"
            "import json, sys\n"
            "args = sys.argv[1:]\n"
            f"remote_branches = {sorted(self.remote_branches) if self.remote_branches is not None else None!r}\n"
            f"rc_overrides = {self.ls_remote_rc_overrides!r}\n"
            "if args[:2] == ['ls-remote', '--exit-code']:\n"
            "    with open(" + repr(str(self.git_calls_file)) + ", 'a') as f:\n"
            "        f.write(json.dumps({'args': args}) + '\\n')\n"
            "    branch = args[-1]\n"
            "    if branch in rc_overrides:\n"
            "        sys.exit(rc_overrides[branch])\n"
            "    if remote_branches is None or branch in remote_branches:\n"
            "        print(f'<ref>\\t{branch}')\n"
            "        sys.exit(0)\n"
            "    sys.exit(2)  # confirmed absent (no matching refs)\n"
            "sys.stderr.write(f'unmocked git argv: {args}\\n'); sys.exit(3)\n"
        )
    ...
    def read_git_calls(self) -> list[dict]:
        """Return recorded mock-git ls-remote invocations as {"args": [...]}."""
        calls = []
        for line in self.git_calls_file.read_text().splitlines():
            if line.strip():
                calls.append(json.loads(line))
        return calls
...
# ============================================================================
# 竞态护栏（Issue #129）：immediate 模式下 cleanup 步算出 protected 清单后、
# tracking-issue 步写单前，分支可能已被删除（auto-merge 删分支与建单竞态，
# 实测 4 秒窗口）。写单前对将写入的分支做远端存活性重验：
#   - CREATE 路径：整个 protected 清单重验后过滤
#   - UPDATE 路径：只重验将新写入的 ADDED_PROTECTED
#   - git ls-remote 失败（rc≠0/2，网络等）：fail-open 保留分支
#   - 过滤后清单空且无删除 → 走零 actionable 路径
# ============================================================================
class TestStalenessGuard:
    """Remote-liveness revalidation before tracker writes (Issue #129)."""

    def test_create_path_drops_absent_branch(self, tmp_path: Path):
        """CREATE 路径：远端已不存在的分支不得写入新建 tracker。"""
        harness = GhMockHarness(
            tmp_path,
            remote_branches={"fix/live-branch"},  # fix/raced-branch absent
        )

        exit_code, _stdout, _stderr = harness.run_script(
            protected=[
                "fix/live-branch (2 unique commits)",
                "fix/raced-branch (4 unique commits)",
            ]
        )
        stdout = _stdout + _stderr

        assert exit_code == 0, stdout
        assert "staleness_guard: dropped fix/raced-branch (no longer on remote)" in stdout
        calls = harness.read_calls()
        creates = calls_matching(calls, ["issue", "create"])
        assert len(creates) == 1, "surviving branch still creates the tracker"
        body_idx = creates[0].args.index("--body") + 1
        body = creates[0].args[body_idx]
        assert "fix/live-branch" in body
        assert "fix/raced-branch" not in body, "absent branch must not enter the tracker body"

    def test_create_path_all_stale_zero_actionable(self, tmp_path: Path):
        """CREATE 路径：全部候选已删除且无 deletion → 零 actionable，不开单。"""
        harness = GhMockHarness(tmp_path, remote_branches=set())

        exit_code, _stdout, _stderr = harness.run_script(protected=["fix/gone (1 unique commits)"])
        stdout = _stdout + _stderr

        assert exit_code == 0, stdout
        assert "staleness_guard: dropped fix/gone (no longer on remote)" in stdout
        assert "issue_action=none" in stdout
        calls = harness.read_calls()
        assert calls_matching(calls, ["issue", "create"]) == [], "no tracker for stale-only run"

    def test_create_path_ls_remote_failure_keeps_branch(self, tmp_path: Path):
        """ls-remote 失败（网络等，rc=128）→ fail-open 保留分支，照常建单。"""
        harness = GhMockHarness(
            tmp_path,
            ls_remote_rc_overrides={"fix/flaky-net": 128},
        )

        exit_code, _stdout, _stderr = harness.run_script(
            protected=["fix/flaky-net (3 unique commits)"]
        )
        stdout = _stdout + _stderr

        assert exit_code == 0, stdout
        assert "staleness_guard: remote check failed for fix/flaky-net" in stdout
        assert "keeping (fail-open)" in stdout
        calls = harness.read_calls()
        creates = calls_matching(calls, ["issue", "create"])
        assert len(creates) == 1, "indeterminate check must not block the tracker"
        body_idx = creates[0].args.index("--body") + 1
        assert "fix/flaky-net" in creates[0].args[body_idx]

    def test_update_path_drops_absent_added_branch(self, tmp_path: Path):
        """UPDATE 路径：仅重验将新写入的 ADDED_PROTECTED，确认缺席即从
        comment 与 body 剔除；幸存的新增分支照常写入；已在 tracker 的旧
        分支不重验。"""
        harness = GhMockHarness(
            tmp_path,
            issues={781: tracker_body(["fix/tracked-old (2 unique commits)"])},
            remote_branches={"fix/tracked-old", "fix/live-add"},  # fix/raced-add absent
        )

        exit_code, _stdout, _stderr = harness.run_script(
            protected=[
                "fix/tracked-old (2 unique commits)",
                "fix/raced-add (5 unique commits)",
                "fix/live-add (3 unique commits)",
            ]
        )
        stdout = _stdout + _stderr
        ...
        assert exit_code == 0, stdout
        assert "issue_action=updated" in stdout
        assert "staleness_guard: dropped fix/raced-add (no longer on remote)" in stdout
        calls = harness.read_calls()
        assert calls_matching(calls, ["issue", "create"]) == []
        comments = calls_matching(calls, ["issue", "comment"])
        assert len(comments) == 1
        comment_idx = comments[0].args.index("--body") + 1
        comment = comments[0].args[comment_idx]
        assert "fix/raced-add" not in comment, "absent added branch must not be commented"
        assert "fix/live-add" in comment, "surviving added branch must be commented"
        # body 亦不得记录缺席分支，且包含幸存分支
        edits = calls_matching(calls, ["issue", "edit"])
        assert len(edits) == 1
        body_idx = edits[0].args.index("--body") + 1
        body = edits[0].args[body_idx]
        assert "fix/raced-add" not in body
        assert "fix/live-add" in body and "fix/tracked-old" in body
        # ADDED 重验只查新增分支；旧分支零重验（竞态窗口语义：仅护栏写入面）
        git_branches = sorted(c["args"][-1] for c in harness.read_git_calls())
        assert git_branches == ["fix/live-add", "fix/raced-add"], (
            f"only the added branches must be revalidated, got {git_branches}"
        )

    def test_update_path_added_all_stale_falls_silent(self, tmp_path: Path):
        """UPDATE 路径：ADDED 全部被剔除且无 REMOVED/删除 → 无评论必要，
        走 reused-silent（不写空评论）。"""
        harness = GhMockHarness(
            tmp_path,
            issues={781: tracker_body(["fix/tracked-old (2 unique commits)"])},
            remote_branches={"fix/tracked-old"},
        )

        exit_code, _stdout, _stderr = harness.run_script(
            protected=[
                "fix/tracked-old (2 unique commits)",
                "fix/raced-add (5 unique commits)",  # absent on remote
            ]
        )
        stdout = _stdout + _stderr
        ...
        assert "issue_action=reused-silent" in stdout
        calls = harness.read_calls()
        assert calls_matching(calls, ["issue", "comment"]) == []
        assert calls_matching(calls, ["issue", "edit"]) == [], (
            "no body rewrite needed when the net protected set is unchanged"
        )

    def test_reused_silent_path_skips_revalidation(self, tmp_path: Path):
        """无变化路径（reused-silent）不重验：竞态护栏只覆盖写入面。"""
        harness = GhMockHarness(
            tmp_path,
            issues={781: tracker_body(["fix/tracked-old (2 unique commits)"])},
            remote_branches=set(),  # even if absent, must NOT be checked
        )

        exit_code, _stdout, _stderr = harness.run_script(
            protected=["fix/tracked-old (2 unique commits)"]
        )
        stdout = _stdout + _stderr

        assert exit_code == 0, stdout
        assert "issue_action=reused-silent" in stdout
        assert harness.read_git_calls() == [], "unchanged run must not hit ls-remote"

    def test_zero_actionable_close_path_skips_revalidation(self, tmp_path: Path):
        """零 actionable 关单路径不重验（无写入面）。"""
        harness = GhMockHarness(
            tmp_path,
            issues={781: tracker_body(["fix/tracked-old (2 unique commits)"])},
            remote_branches=set(),
        ...
        assert exit_code == 0, stdout
        assert "issue_action=closed" in stdout
        assert harness.read_git_calls() == []

    def test_deletions_still_reported_when_remote_empty(self, tmp_path: Path):
        """删除事件不受护栏影响：DELETED_ITEMS 不重验，照常建单报告。"""
        harness = GhMockHarness(tmp_path, remote_branches=set())

        exit_code, _stdout, _stderr = harness.run_script(deleted=["feat/merged-and-gone"])
        stdout = _stdout + _stderr
        ...
        creates = calls_matching(calls, ["issue", "create"])
        body_idx = creates[0].args.index("--body") + 1
        assert "feat/merged-and-gone" in creates[0].args[body_idx]
        assert harness.read_git_calls() == [], "deletions are never revalidated"

    def test_create_path_dedupes_entries(self, tmp_path: Path):
        """重复条目去重：同名分支只重验一次、只写一行。"""
        harness = GhMockHarness(tmp_path, remote_branches={"fix/dup-branch"})

        exit_code, _stdout, _stderr = harness.run_script(
            protected=[
                "fix/dup-branch (2 unique commits)",
                "fix/dup-branch (2 unique commits)",
            ]
        )
        stdout = _stdout + _stderr
        ...
        assert exit_code == 0, stdout
        git_calls = harness.read_git_calls()
        assert len(git_calls) == 1, f"duplicate entries must be revalidated once: {git_calls}"
        calls = harness.read_calls()
        creates = calls_matching(calls, ["issue", "create"])
        body_idx = creates[0].args.index("--body") + 1
        assert creates[0].args[body_idx].count("fix/dup-branch") == 1
